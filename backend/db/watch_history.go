...
func (d *Database) RecordOnlineWatch(e WatchHistoryEntry) error {
	_, err := d.conn.Exec(`
		INSERT INTO watch_history
			(source_id, source_name, anime_id, anime_title, cover_url,
			 episode_id, episode_num, episode_title, duration_s, completed, hidden, watched_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, FALSE, CURRENT_TIMESTAMP)
		ON CONFLICT(source_id, episode_id) DO UPDATE SET
			watched_at   = CURRENT_TIMESTAMP,
			duration_s   = excluded.duration_s,
			completed    = CASE
				WHEN watch_history.completed = TRUE AND excluded.completed = FALSE THEN TRUE
				ELSE excluded.completed
			END,
			hidden       = FALSE
	`,
		e.SourceID, e.SourceName, e.AnimeID, e.AnimeTitle, e.CoverURL,
		e.EpisodeID, e.EpisodeNum, e.EpisodeTitle, e.DurationSec, e.Completed,
	)
	return err
}
...
func (d *Database) GetRecentlyWatched(limit int) ([]WatchHistoryEntry, error) {
	rows, err := d.conn.Query(`
		SELECT w.id, w.source_id, w.source_name, w.anime_id, w.anime_title,
		       COALESCE(w.cover_url, ''), w.episode_id, COALESCE(w.episode_num, 0),
		       COALESCE(w.episode_title, ''), w.watched_at, w.duration_s, w.completed
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&e.ID, &e.SourceID, &e.SourceName, &e.AnimeID, &e.AnimeTitle,
			&e.CoverURL, &e.EpisodeID, &e.EpisodeNum, &e.EpisodeTitle,
			&watchedAt, &e.DurationSec, &e.Completed,
		); err != nil {
			continue
		}
...
func (d *Database) GetAnimeWatchHistory(sourceID, animeID string) ([]WatchHistoryEntry, error) {
	rows, err := d.conn.Query(`
		SELECT id, source_id, source_name, anime_id, anime_title,
		       COALESCE(cover_url, ''), episode_id, COALESCE(episode_num, 0),
		       COALESCE(episode_title, ''), watched_at, duration_s, completed
		FROM watch_history
