...
assert.equal(
  normalizeReaderSettings({ pageFit: 'cover' }).pageFit,
  'cover',
)
...
assert.deepEqual(pagedPortraitAtDefaultZoom, { width: 616, height: 924 })
assert.deepEqual(pagedPortraitAtHighZoom, { width: 684, height: 1026 })
assert.ok(
...
assert.ok(/\.reader-spread\s*\{[^}]*width: 100%;[^}]*min-height: 100%;[^}]*height: fit-content;/s.test(cssSource), 'reader spread should own the available viewport height while allowing immersive overflow')
