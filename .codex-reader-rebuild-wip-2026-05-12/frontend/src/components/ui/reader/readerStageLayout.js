...
function getFitScale(pageFit = 'width', zoomPercent = DEFAULT_ZOOM) {
  ...
  if (pageFit === 'height') return 0.96 + (0.18 * zoomProgress)
  if (pageFit === 'original') return 0.88 + (0.26 * zoomProgress)
  if (pageFit === 'cover') return 1.0 + (0.18 * zoomProgress)
  return 0.97 + (0.17 * zoomProgress)
}
