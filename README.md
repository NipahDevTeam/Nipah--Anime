...
OAuth credentials are not included in this repository. See `.env.example` for the required environment variables and how to register your own AniList and MyAnimeList OAuth apps for local development.
...
Las credenciales OAuth no están incluidas en este repositorio. Consulta `.env.example` para ver las variables de entorno necesarias y cómo registrar tus propias apps OAuth de AniList y MyAnimeList para desarrollo local.
